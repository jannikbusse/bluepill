...
void mod_init_mod_connections()
{
    ...
    mod_init_mod_connection(&oscilator[0].mp_mod_pitchLfo,  &(lfos[0].current_scalar),    0.2f,      MOD_CONNECTION_SIDE_CENTER);
...
{
    in->pot[POT_ID_VOLUME].target = &(mp_lp_baseCutOffFreq);
    in->pot[POT_ID_VOLUME].outputOffset = 0.01f;
    in->pot[POT_ID_VOLUME].outputRange = 20000;
}
