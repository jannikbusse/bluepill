...
void env_init_env(envelope *env)
{
    ...
    env->attack_duration = 0.51f;
    env->decay_duration = 0.4f;
    env->sustainCeiling = 0.8f;
    env->release_duration = 0.9f;
...
void env_update_envelope(envelope *env, key_event event)
{
    ...
    }else
    {
        ...
        {
        case ENV_ATTACK:
            env->current_scalar = env->attack(env->time_since_last_state_change, env->attack_duration);
            ...
            break;
        case ENV_DECAY:
            env->current_scalar = env->decay(env->time_since_last_state_change, env->decay_duration, env->sustainCeiling);
        ...
        case ENV_SUSTAIN:
            ...
            break;
        case ENV_RELEASE:
            env->current_scalar = env->release(env->time_since_last_state_change, env->release_duration, env->sustainCeiling);
